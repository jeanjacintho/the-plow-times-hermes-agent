...
STAGE_OK = {"stage": "blueprint", "modifiers": [], "domain": "b2b-saas",
            "signals": ["arr=2.4M"], "why": "ARR 2.4M is the $1–10M band",
            "label": "Blueprint ($1–10M ARR)"}
ADV_OK = {"advisors": [{
    "file": "blueprint.md", "advisor": "Patrick Salyer (Mayfield)",
    "stages": ["blueprint"], "domain": "b2b-saas-enterprise", "source": "",
    "sections": [{"id": "focus-first", "kind": "focus", "heading": "Focus first", "text": "Write the playbook"}],
}], "errors": []}


def test_build_ok():
    c = bc.build("2026-09-16", "America/Sao_Paulo", FILE_OK, CAL_OK, [], STAGE_OK, ADV_OK)
    ...
    assert c["stage"]["stage"] == "blueprint" and len(c["advisors"]) == 1
...
def test_notes_and_defaults():
    ...
    assert c["notes"] == ["file_missing", "calendar_unavailable", "stage_unknown", "no_advisor_for_stage"]
    assert not bc.has_input(c)
    c = bc.build("2026-09-16", "UTC", {"status": "empty", "truncated": False, "sections": []}, CAL_OK, [])
    assert "file_empty" in c["notes"] and bc.has_input(c)
...
def test_cli_reads_run_dir_and_history(pt_home, tmp_path, capsys):
    ...
    (run / "desk-priority" / "stage.json").write_text(json.dumps(STAGE_OK))
    (run / "desk-priority" / "advisors.json").write_text(json.dumps(ADV_OK))
    hist.record("2026-09-15", "Ship pricing page")
    bc.main(["--run-dir", str(run), "--tz", "UTC", "--now", "2026-09-16T12:00:00+00:00"])
    assert capsys.readouterr().out.strip() == "CONTEXT:ok STAGE:blueprint NOTES:none"
...
def test_cli_calendar_unavailable_without_day_json(pt_home, tmp_path, capsys):
    ...
    (run / "desk-priority" / "advisors.json").write_text(json.dumps(ADV_OK))
    bc.main(["--run-dir", str(run), "--tz", "UTC", "--now", "2026-09-16T12:00:00+00:00"])
    assert capsys.readouterr().out.strip() == "CONTEXT:ok STAGE:blueprint NOTES:calendar_unavailable"
