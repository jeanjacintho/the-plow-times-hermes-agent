...
2. The company record: `/var/lib/hermes/skills/pt-priority/scripts/company.py show`. Only
   when it prints `EMPTY`, bootstrap it once from the Mac, read-only and bounded:
   ...
   pt-priority records the facts. A deny or an error: go on without them. After the first
   day, today's messages and mail (steps 4–5) keep the record current.
