...
roster and leave the setting unchanged. A file that still has
`delivery.local_hour` stored its hours on the container's clock: drop that
key in the same write. After a valid change, re-run the gate
and then re-run
