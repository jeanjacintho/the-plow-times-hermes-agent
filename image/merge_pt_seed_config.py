...
provider and terminal.cwd onto the home every boot. The same merge runs onto
an existing home at boot (cont-init.d/03-merge-pt-runtime-config). Merging
every key keeps the Opus 5 pin, the quiet plow_chat block and the rest of
runtime/config.yaml (cron.wrap_response, agent.disabled_toolsets,
context_file_max_chars, ...) from falling back to the base's on any path. Mappings merge; any other
