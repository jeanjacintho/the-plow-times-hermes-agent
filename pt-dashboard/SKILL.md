...
| job | schedule | notes |
|---|---|---|
| `pt-daily-edition` | `<min> <hour> * * *`, computed as `delivery.hour − delivery.lead_minutes` (default 60) in the owner's zone, never before that day's midnight | one job; the **main** paper: desks, sections with no `deliver_at` (or `deliver_at` equal to this hour), and assignments due today |
...
The daily schedule is computed in minutes and stops at midnight, so
`00:30 − 60min` is `0 0 * * *`: a run started the evening before would be
the previous day's run. `00:00` is a real delivery hour, fired at midnight.
