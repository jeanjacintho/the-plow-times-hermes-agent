...
def daily_prompt(lock_name, live=False):
    ...
    return (
        ...
        f"/var/lib/hermes/skills/pt-intake/scripts/topics.py list and count the "
        f"active main-paper sections plus assignments due on or before today. "
        f"If there are more than three, name every news item and stop before research; "
        f"the owner must drop enough to restore the one-page roster. Then run pt-research: first "
...
def paper_prompt(lock_name, hour):
    ...
    return (
        ...
        f"(delivered sections are yesterday's paper, not a skip). Run "
        f"/var/lib/hermes/skills/pt-intake/scripts/topics.py list and count the "
        f"active sections for {hour}. If there are more than three, name every news item "
        f"and stop before research; the owner must drop enough to restore the one-page "
        f"roster. Then run pt-research: first "
        f"every standing desk pt-research/references/desks.md lists, in its order, "
