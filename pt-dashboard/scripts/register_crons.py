...
def daily_prompt(lock_name):
    ...
    return (
        ...
        f"(delivered sections are yesterday's paper, not a skip). Then run pt-research: first the "
        f"standing desks (location via Latch then weather; calendar via "
        f"plow-gog: calendar calendars then selected events --from now; "
        f"mail only if pt/config.json has mail.configured true — "
        f"Gmail via plow-gog), "
